    ...
    <div class="meetup-form__content">
      <fieldset class="meetup-form__section">
        <ui-form-group label="Название">
          <ui-input v-model="meetupLocal.title" name="title" />
        </ui-form-group>
        <ui-form-group label="Дата">
          <ui-input-date v-model="meetupLocal.date" type="date" name="date" />
        </ui-form-group>
        <ui-form-group label="Место">
          <ui-input v-model="meetupLocal.place" name="place" />
        </ui-form-group>
        <ui-form-group label="Описание">
          <ui-input v-model="meetupLocal.description" multiline rows="3" name="description" />
        </ui-form-group>
...
export default {
  ...
  data() {
    const meetupLocal = cloneDeep(this.meetup);
    return {
      meetupLocal: meetupLocal,
    };
  },
  ...
  methods: {
    ...
    addAgendaItem() {
      ...
      const lastAgenda = last(this.meetupLocal.agenda);
      if (lastAgenda) {
        newAgenda.startsAt = lastAgenda.endsAt;
        newAgenda.endsAt = lastAgenda.endsAt;
      }
      this.meetupLocal.agenda.push(newAgenda);
    },

    removeAgendaItem(idx) {
      this.meetupLocal.agenda.splice(idx, 1);
    },

    updateAgendaItem(idx, val) {
      this.meetupLocal.agenda[idx] = val;
    },
